PROMPT 013 – DERIVE PRODUCT GROUP

Objective

Generate executable Snowflake SQL that derives DERIVED_PRODUCT_GROUP for active master records and generates UPDATE actions into ACTION_LOG.
...
The BR is responsible only for deriving DERIVED_PRODUCT_GROUP and generating UPDATE actions.
...
sql/br/013_derive_product_group.sql
...
DERIVED_PRODUCT_FAMILY is already a mastered attribute.

DERIVED_PRODUCT_GROUP shall be derived from DERIVED_PRODUCT_FAMILY using a deterministic CASE statement.
...
DERIVED_PRODUCT_GROUP must be derived from DERIVED_PRODUCT_FAMILY.

The BR must not derive PRODUCT_GROUP directly from CATEGORY.

---

Derivation Logic

DERIVED_PRODUCT_FAMILY Values:
...
Phase 2 – Derive Product Group

Apply the approved CASE statement.

Create:

NEW_DERIVED_PRODUCT_GROUP
...
COALESCE(DERIVED_PRODUCT_GROUP,'')
<>
COALESCE(NEW_DERIVED_PRODUCT_GROUP,'')
...
* same ACTION_TYPE
* same ACTION_SOURCE
* same DERIVED_PRODUCT_GROUP value
...
INTM_PRODUCT_MASTER
↓
Read DERIVED_PRODUCT_FAMILY
↓
Derive Product Group
↓
ACTION_LOG (UPDATE)
↓
P11 DAL
